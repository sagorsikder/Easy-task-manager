...
const MyTask = () => {
    ...
    console.log(task)
    useEffect(()=>{
        fetch(` https://easy-task-manager-server.vercel.app/mytask?email=${user.email}`)
        .then(res=> res.json())
   ...
   const handleComplete=(id)=>{
        ...
        fetch(` https://easy-task-manager-server.vercel.app/update1?id=${id}`,{
            method:'POST',
            headers:{
   ...
   const handleDelete=(id)=>{
    ...
    return (
        ...
        <h1 className='text-center lg:text-5xl font-bold  text-white pt-10 mb-5'>My tasks </h1>
       ...
       
       {
        task.map(ts=> <div className='rounded bg-slate-500 mb-5 p-3 text-white relative'>
             ...
             <button onClick={()=>handleComplete(ts._id)} className='bg-blue-500 hover:bg-blue-700 py-1 px-2 absolute top-2 right-2'>Complete</button>
